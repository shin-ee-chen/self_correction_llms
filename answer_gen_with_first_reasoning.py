...
def parse_args():
    ...
    parser.add_argument("--dataset_dir", default="/projects/0/gusr0608/self_correction_llms/modified_first_reasoning/aime24_wrong_first_reasoning.json", type=str)
    ...
    parser.add_argument("--use_alternatively", action="store_true")
...
def prepare_data(data_path, data_name, args):
    ...
    generated_dataset_file = f"{output_dir}/{data_name}/predictions/{out_file_prefix}_num{args.num_test_sample}s{args.start}e{args.end}_{args.use_alternatively}_dataset_predictions.json"
    ...
    for i, sample in enumerate(samples):
        for j in range(len(sample['reasonings'])):
            for _ in range(args.n_sampling):
                prompts.append(sample['prompt'] + sample['reasonings'][j] + " Alternatively, " if args.use_alternatively else "")
